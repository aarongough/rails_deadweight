...
Trying to remove everything that Rails Deadweight finds in one go is probably not advisable, because if you run up against test failures it will be very difficult to find out which part of your change caused the failure.

Rails Deadweight does its best to minimize false positives, but because of the highly dynamic nature of Ruby there are no absolute guarantees that its results will be correct.
